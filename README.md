...
| 脚本 | 用途 | 详细说明 |
|---|---|---|
| `manager.py` | 笔记工作区管理面板，并作为**总入口**调用其他脚本 | `manager.md` |
| `symbols.py` | 符号库管理：提取 / 回填 / 刷新补全 / 分发 | 见下 |
| `progress.py` | 写作进度追踪：扫描笔记并在浏览器中查看 | `progress.md`、`progress_design.md` |
| `check_sensitive.py` | 提交前扫描本机信息（用户名 / 本机路径 / 专有词） | 见下 |
| `commit.py` | 一键提交：`add → commit → push`，含 `--check` 模式供 git 钩子调用 | `commit.md` |
...
| 选项 | 功能 |
|---|---|
| 1 | 检查 git 安装、用户名与邮箱、SSH 密钥及 443 端口配置，并实测与 GitHub 的连通性 |
| 2 | 交互式设置全局 `user.name` 与 `user.email`（回车保持不变） |
| 3 | 生成 ed25519 密钥、向 `~/.ssh/config` 写入 `github.com → ssh.github.com:443`、显示公钥 |
| 4 | 设置笔记根目录并扫描子文件夹，识别其中的 git 仓库 |
| 5 | 列出各仓库分支、改动文件数、相对远程的领先/落后提交数、远程仓库名；并列出尚未入版本控制的文件夹 |
| 6 | 勾选仓库后批量执行 `add → commit → push`；推送失败会自动重试并尝试备用端口 |
| 7 | 为未入版本控制的文件夹建仓库：补 `.gitignore` 与 `.gitattributes`、`git init`、首次提交并推送 |
| 8 | 退出 |
| 9 | 进入 `symbols.py` 的符号库管理面板，退出后返回本面板 |
| 10 | 选一本笔记后，调用其目录下的 `commit.py`（提交并推送）或 `setup_mode.py`（切换习题编排模式） |
| 11 | 后台启动「写作进度追踪表」的本地服务并打开浏览器，面板立即返回 |

选项 9、10、11 是**子工具入口** —— 不重复实现功能，只调用同目录下的已有脚本，各脚本职责保持不变。
