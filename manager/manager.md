...
笔记工作区管理面板：统一管理笔记根目录下各子文件夹的仓库 —— 查看状态、批量提交推送、为尚未入版本控制的文件夹创建仓库。
...
| 2 | 列出各仓库的分支、改动文件数、相对远程的**领先/落后提交数**、远程仓库名；并列出尚未入版本控制的子文件夹 |
| 3 | 先让用户勾选要操作的仓库（默认只列有改动的），再统一执行 `add → commit → push`；可选择「只推送、不提交」 |
